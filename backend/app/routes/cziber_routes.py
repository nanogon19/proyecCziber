...
def agregar_conexion():
    """
    Crear una nueva conexión en el sistema con valores por defecto
    """
    try:
        data = request.json
        
        # Validar datos requeridos (solo los básicos de conexión)
        required_fields = ["ip", "port", "database_name", "username", "password"]
        for field in required_fields:
            if field not in data:
                return jsonify({"error": f"Campo requerido faltante: {field}"}), 400
        
        # Obtener valores por defecto automáticamente
        # Obtener la primera empresa disponible
        company = Company.query.first()
        if not company:
            return jsonify({"error": "No hay empresas registradas en el sistema"}), 404
            
        # Obtener la primera aplicación disponible
        app = Application.query.first()
        if not app:
            return jsonify({"error": "No hay aplicaciones registradas en el sistema"}), 404
            
        # Obtener el primer modelo disponible
        model = Model.query.first()
        if not model:
            return jsonify({"error": "No hay modelos registrados en el sistema"}), 404
        
        print(f"Usando valores por defecto - Empresa: {company.name}, Aplicación: {app.nombre}, Modelo: {model.nombre}")
        
        # Crear nueva conexión con valores por defecto
        ...
        return jsonify({
            ...
            "defaults_used": {
                ...
                "modelo": f"{model.nombre} v{model.version}"
            }
        }), 201
